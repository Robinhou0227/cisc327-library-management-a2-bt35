...
def get_patron_borrow_history(patron_id: str) -> List[Dict]:
    """Get complete borrow history for a patron (including returned books)."""
    conn = get_db_connection()
    records = conn.execute('''
        SELECT br.*, b.title, b.author 
        FROM borrow_records br 
    ...
    for record in records:
        history.append({
            ...
            'is_overdue': datetime.now() > datetime.fromisoformat(record['due_date']) if not record['return_date'] else False
        })
